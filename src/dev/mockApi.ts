...
export function installDevMockApi() {
  ...
  const defaultTheme = "LuminaPlusForPTer";
    ...
    if (url.pathname === "/api/public") {
      ...
      return json({
        ...
        theme_settings: savedThemeSettings[theme] ?? {
          // ?viewMode=large|compact|mini|list 直接锁定预览视图,省去点浮动按钮切换。
          desktopNodeViewMode:
            new URLSearchParams(window.location.search).get("viewMode") ?? "compact",
          mobileNodeViewMode: "compact",
          showHomeOverview: true,
          showGroupTabs: true,
          showRegionBar: true,
          showCardGroup: true,
          // 厂商 LOGO 预览默认打开;在线 ASN 查询在 mock 下关闭，避免 dev 向公网发包。
          showProviderLogo: true,
          providerOnlineLookup: false,
          enableHomeSort: true,
