...
export interface TodayTrafficStatsContentProps {
  /** page = 独立统计页(带返回)；dialog = 首页弹窗(带标题与关闭)。 */
  headerMode: "page" | "dialog";
  onClose?: () => void;
}

/**
 * 今日流量统计主体：汇总卡 + 节点明细，页面与弹窗共用同一份渲染与查询逻辑。
 * 查询在组件挂载时发起(full 模式)，与首页卡片共享 queryKey 缓存。
 */
export function TodayTrafficStatsContent({
  headerMode,
  onClose,
}: TodayTrafficStatsContentProps) {
  ...
  const isDialog = headerMode === "dialog";

  return (
    <div className={isDialog ? "traffic-dialog-content" : "traffic-page flex flex-col gap-4 py-2"}>
      <div className={isDialog ? "traffic-dialog-head" : "flex items-center justify-between gap-3"}>
        {isDialog ? (
          <h2 id="today-traffic-dialog-title" className="traffic-dialog-title">
            今日流量详情
          </h2>
        ) : (
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {isDialog && (
            <button
              type="button"
              className="traffic-dialog-close"
              onClick={onClose}
              aria-label="关闭今日流量详情"
              title="关闭"
            >
                  ...
                  {stat.hasSamples && (
                    <>
                      <div className="traffic-node-card-directions">
                        <span>↑ 峰值 {formatByteRateLabel(stat.peakUp)}</span>
                        <span>↓ 峰值 {formatByteRateLabel(stat.peakDown)}</span>
                      </div>
                      <dl className="traffic-node-card-peaks">
                        <div>
                          <dt>上行流量</dt>
                          <dd><strong>{formatBytes(stat.trafficUp)}</strong></dd>
                        </div>
                        <div>
                          <dt>下行流量</dt>
                          <dd><strong>{formatBytes(stat.trafficDown)}</strong></dd>
                        </div>
                      </dl>
                    </>
                  )}
